import React from "react";
import { Link } from "react-router-dom";
import "./stylefor/People.css"

const People = ({ peopleResult, peopleTotalPage, peopleCurrentPage, setPeopleCurrentPage }) =>{
    const imgBaseURL = "https://image.tmdb.org/t/p/w200";
    return(
        <>
            {
                peopleResult && peopleResult.map((current)=>{
                    return(
                        <>
                            <div className="peopleContainer">
                                <Link to = {`/person/${current.id}/${current.name}`} className="peopleImg">
                                    {
                                        current.profile_path == null ? <img src="data:image/png;base64," alt="" /> :
                                        <img src={`${imgBaseURL}${current.profile_path}`} alt={current.name} />
                                    }
                                   </Link>
                                <div >
                                    <div><Link className = "PeopleContainerLink" to={`/person/${current.id}/${current.name}`} style={{fontWeight:"bold"}}>{current.name}</Link></div>
                                    <div >
                                        <span>{`${current.known_for_department}.`} </span>

                                            {
                                                current.known_for && current.known_for.map((knownFor)=>{
                                                    return(
                                                        <>
                                                            <Link className = "PeopleContainerLink" to={`/${knownFor.title || knownFor.name}/${knownFor.id}`}>
                                                                {knownFor.title || knownFor.name}
                                                            </Link>
                                                        </>
                                                    )
                                                })
                                            }
                                    </div>
                                </div>
                            </div>
                        </>
                    )
                })
            }
            <div style={{display:"flex", alignItems:"center", justifyContent:"center"}}>
                {peopleCurrentPage < peopleTotalPage && <button className= "lodeMoreBtn" 
                        onClick={() => {
                            setPeopleCurrentPage(peopleCurrentPage + 1);
                        }}
                        >
                        Load More
                </button>}    
            </div>
            
            
        </>
    )
}

export default People;